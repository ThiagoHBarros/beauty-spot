...
module.exports = {
    IAM_USER_KEY: 'AKIAZH5DASLHNGWWTOMI',
    IAM_USER_SECRET: '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',
    BUCKET_NAME: 'beauty-spot-dev',
    ...
    uploadToS3: function(file, filename, acl = 'public-read'){
        return new Promise((resolve, reject) => {
            ...
            s3bucket.upload(params, function(err, data){
                if(err){
                    console.log(err);
                    return resolve({ error: false, message: err.message })
                }
                console.log(data);
